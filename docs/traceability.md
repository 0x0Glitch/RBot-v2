...
| 16.1–16.6 | Transaction-complete attribution and exclusive FIFO idle-lock ledger | `src/state/attribution.rs`, `src/state/idle_locks.rs`, JSON state document | ordered replay, exact-balance, lock-kind and uncertainty tests | Implemented |
...
| 24.2–24.4 | Same-head exact rebuild, three-scenario hard-constraint validation, simulation, gas bound, exclusive dependencies, durable movement/nonce/signing order and signed-byte submission | `src/runtime/preflight_source.rs`, `src/transaction/final_preflight.rs`, `src/chain/provider.rs` | real local-chain source through success plus post-persistence head-move integration tests and reservation unit test | Implemented and invoked by the supervised Execute controller |
...
| 26, 21 | Explicit per-vault runtime state graph and mode-aware fail-closed readiness | `src/runtime/{controller,readiness,state_service,planning_service,execution_service}.rs`, `src/main.rs` | runtime transition/readiness tests and local deployed-state/Shadow/Execute E2E | Canonical chain/state/plan/API and non-mainnet local-development Execute composition implemented; production remote signer remains fail closed |
