...
| 11.9–11.17 | Frozen-direction rate episodes with non-rearming cumulative budgets and durable recovery | `src/planner/episodes.rs`, `src/runtime/planning_service.rs`, `src/storage/actor.rs` | budget, uniqueness, reopen, direct-parent live confirmation and reorg-reversal tests | Immediate episode path implemented; persistent independent-event unlock remains live-runtime work |
...
| Milestone 12 | Deterministic local-chain vertical slice through terminal reconciliation | `tests/fixtures/e2e`, `tests/local_e2e.rs` | Forge deployment; supervised deployed-identity/canonical-state and durable Shadow-plan path; deposit/allocation; atomic snapshots; bounded rate solve; live exact preflight source; firewall, real-EOA call, signing and submission; execution-controller confirmation; exact event conformance; improved rate; live current-state reconciliation; cap refresh; adapter removal hard pause/re-add replanning; JSON restart | Main rate and administration-invalidation paths implemented; replacement/cancellation and broader crash/reorg matrix pending |
