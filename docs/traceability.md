...
| 26, 21 | Explicit per-vault runtime state graph and mode-aware fail-closed readiness | `src/runtime/{controller,readiness,state_service,planning_service}.rs`, `src/main.rs` | runtime transition/readiness tests and local deployed-state/Shadow-plan E2E | Canonical chain/state/Shadow-plan/API composition implemented; signer remains false and nonzero idle fails closed until execution services attach |
...
| Milestone 12 | Deterministic local-chain vertical slice through terminal reconciliation | `tests/fixtures/e2e`, `tests/local_e2e.rs` | Forge deployment; supervised deployed-identity/canonical-state and durable Shadow-plan path; deposit/allocation; atomic snapshots; bounded rate solve; firewall, real-EOA call, signing and submission; confirmation; exact event conformance; improved rate; current-state reconciliation; cap refresh; adapter removal hard pause/re-add replanning; JSON restart | Main rate and administration-invalidation paths implemented; supervised executor and broader crash/reorg matrix pending |
